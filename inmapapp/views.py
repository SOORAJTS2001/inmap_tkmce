...
def index(request):
    ...
    if request.method == "GET":
        ...
        m = Maze(os.path.abspath('inmapapp/static/inmapapp/mapped_tkmce.txt'),From_x,From_y,To_x,To_y,"tkm_map_normal.jpg")
        m.solve()
        ...
        for sol in solutions[1]:
            ans+=f'''<span class="animationEl" style="width:10px;height:10px;position: absolute;top: {sol[0]*5.35}px;left: {sol[1]*4.75}px;"> </span>'''
        ...
        now = time.time()   
        return render(request,'inmapapp/result.html',{'image':"inmapapp/tkm_map_normal.jpg","time":now-then,"update":True,"data":ans}) 
            # print(From_x,From_y,To_x,To_y)
...
def sample(request):
    ans=""
    for sol in solutions[1]:
        ans+=f'''<span style="width:10px;height:10px;position: absolute;top: {sol[0]*4.5}px;left: {sol[1]*4.2}px;"> </span>'''
    return render(request,'inmapapp/sample.html',{"data":ans})
def test(request):
